...
#open(curl(paste0("https://api.telegram.org/bot456323915:AAGJ5iwFmo69ba3tC2UFr9QK9P2Cm3lNTw8/sendMessage?chat_id=-315767451&text=proses data sales topman ","Start")), "rb", blocking = FALSE)
...
#spark count row
#prev_year_sales_sc %>% sdf_nrow()
# current_year_sales_sc <- current_year_sales_sc %>% sdf_register("current_year_sales_sc")
# tbl_cache(sc, "current_year_sales_sc")

# Register 
#data_current_year_agg <- current_year_sales_tbl
# DO NOT USER CACHE -> WRONG DATA RETURN
#data_current_year_agg <- current_year_sales_tbl %>% sdf_register("data_current_year_agg")
#tbl_cache(sc, "data_current_year_agg")
# https://stackoverflow.com/questions/44726552/what-is-the-most-efficient-way-to-create-new-spark-tables-or-data-frames-in-spar
# sdf_register is not very useful when dealing with long running queries. 
# It is basically an unmaterialized view which means it runs the underlying query each time you call it.
...
for (row in 1:nrow(loop_date)) {
    ...
    # # TESTING
    # ytd_month <- current_year_sales_sc %>% 
    #   filter(between(tanggal, as.Date("2019-08-10"),as.Date("2019-08-10")),
    #          directorate_name=="SUPERMARKET",
    #          store_desc=="S60") %>% 
    #   group_by(directorate_name) %>% 
    #   select(directorate_name,
    #          netsales,qty,sales,cogs,ppn,profit) %>% 
    #   summarise_all(sum) %>% 
    #   ungroup() %>% 
    #   collect()
    ...
    #####################################################################
    # Filter MTD CURRENT YEAR
    # dari tanggal current sampai awal tahun currunt
    # Misal Tanngal current             : 20/05/2019
    # Filter untuk YTD di tahun current : 20/05/2019 s/d 01/01/2019 (First Month)
    #####################################################################
...
#open(curl(paste0("https://api.telegram.org/bot456323915:AAGJ5iwFmo69ba3tC2UFr9QK9P2Cm3lNTw8/sendMessage?chat_id=-315767451&text=proses data sales topman ","selesai ")), "rb", blocking = FALSE)
